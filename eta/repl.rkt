...
(require "eval.rkt" "utils/console.rkt" "parser/parser.rkt" "parser/ast.rkt")
...
(define (repl-loop)
  (prompt)
  (let ((input (read-line)))
    (cond ((eof-object? input) (displayln (colorize "\n👋 Bye!" 'yellow)) (exit)) 
      ((string=? input ":ast") (displayln "Current AST Mode: Debug") (repl-loop))

      (else
        (let ((ast
  (with-handlers
    ((exn:fail? (lambda (e)
  (displayln (colorize (format "Parse error: ~a" (exn-message e)) 'red))
  (repl-loop)
)))
    (parse input)
  )
  ...
  (with-handlers
    ((exn:fail? (lambda (e)
  (displayln (colorize (format "Evaluation error: ~a" (exn-message e)) 'red))
  #f
)))
    (eta-eval ast)
  )
            ...
            (when result (displayln (bold "=> ")) (displayln (colorize (pretty-print-Expr
             result) 'cyan)))
          )
